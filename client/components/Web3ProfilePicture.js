...
export default function Web3ProfilePicture({ isSubscribe, userInfos, pseudo, address, listNftUser, balance, isBlack }) {
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <View style={{ marginLeft: 8 }}>
                                <Text style={{ fontSize: 16, fontWeight: '700', color: isBlack ? 'black' : 'white' }}>{pseudo}</Text>
                                <Text style={{
                                    fontSize: 13,
                                    fontWeight: '400',
                                    color: isBlack ? 'black' : 'white'
                                }}>{address?.slice(0, 5) + "..." + address?.slice(-4)}</Text>
                                <Text style={{ fontSize: 13, fontWeight: '400', color: isBlack ? 'black' : 'white' }}>Nft possedés
